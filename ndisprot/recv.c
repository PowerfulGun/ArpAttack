...
--*/
{
           ...
           !NPROT_IS_LIST_EMPTY(&pOpenContext->RecvPktQueue))
    {
        ...
        // 归还给网卡驱动，让它释放。
        if (NdisGetPoolFromPacket(pRcvPacket) != pOpenContext->RecvNetBufferListPool)
        {
...
--*/
{
    ...
    do
    {
        ...
        NdisAllocatePacket(&Status, &pNdisPacket, pOpenContext->RecvNetBufferListPool);
...
--*/
{
    ...
    if (NdisGetPoolFromPacket(pNdisPacket) == pOpenContext->RecvNetBufferListPool)
    {
        //
