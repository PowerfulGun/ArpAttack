...
--*/
{
    ...
    do
    {
        ...
        // Nonpaged空间。
        NPROT_ALLOC_MEM(pOpenContext, sizeof(NDIS_OPEN_CONTEXT));
        ...
        NPROT_ZERO_MEM(pOpenContext, sizeof(NDIS_OPEN_CONTEXT));
...
#pragma prefast(suppress: 12009, "DeviceName length will not cause overflow")                   
        *(PWCHAR)((PUCHAR)pOpenContext->DeviceName.Buffer + BindingInfoLength) = L'\0';        
        ...
        NdisAllocateBufferPool(
            &Status,
            &pOpenContext->RecvNetBufferPool,
...
--*/
{
    ...
    {
        NdisFreePacketPool(pOpenContext->SendNetBufferListPool);
        pOpenContext->SendNetBufferListPool = NULL;
    }

    if (pOpenContext->RecvNetBufferListPool != NULL)
    {
        NdisFreePacketPool(pOpenContext->RecvNetBufferListPool);
        pOpenContext->RecvNetBufferListPool = NULL;
    }

    if (pOpenContext->RecvNetBufferPool != NULL)
    {
        NdisFreeBufferPool(pOpenContext->RecvNetBufferPool);
        pOpenContext->RecvNetBufferPool = NULL;
    }
...
--*/
{
    ...
    do
    {
        ...
        for (pEnt = Globals.OpenList.Flink;
             ...
             pEnt = pEnt->Flink)
        {
            pOpenContext = CONTAINING_RECORD(pEnt, NDIS_OPEN_CONTEXT, Link);
            NPROT_STRUCT_ASSERT(pOpenContext, oc);
